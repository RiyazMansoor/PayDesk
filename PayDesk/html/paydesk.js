...
//    body += '<div id="waitmsg" /><div id="datadiv"><table id="centereddatatable" class="centereddatatable"></table></div>';
    body += '<div id="waitmsg" /><div id="datadiv"></div>';
    $( 'body' ).html( body );
...
function CenteredDataTableCallback( timer, json )
{
    ...
    var htmldata = '<span class="tablecaption">' + ( settings.caption || settings.title ) + downloadlabel + '</span><br><br><br>';
    //<table id="centereddatatable" class="centereddatatable"></table>
    for ( var tblI = 0 ; tblI < settings.tables.length ; tblI++ )
    {
        htmldata += '<table id="centereddatatable" class="centereddatatable">' + genTable( json, settings.tables[tblI] ) ;
        //if ( tblI == 0 ) htmldata += "<caption>" + ( settings.caption || settings.title ) + downloadlabel + "</caption>" ;
...
function MenuPageReady()
{
    ...
    var tags = menu.pages.reduce( function ( rv, cv ) { return rv.concat( cv.tags ); }, [] );
    console.log( tags );
    tags = tags.filter( function ( item, pos ) { return tags.indexOf( item ) == pos; } ) ;
    tags.sort();
    var tagsoptions = tags.reduce( function ( rv, cv ) { return rv + '<option value="' + cv + '">' + cv + '</option>'; }, '' );
    var pages = menu.pages.reduce( function ( rv, cv ) { return rv + '<tr tags="' + cv.tags.join( ' ' ) + '"><td><span class="menu" url="' + cv.value.toLowerCase() + '">' + cv.name + '</span></td></tr>'; }, '' );
    ...
    body += '<table id="menutable" class="menutable"><tr><td><select id="selTags" class="textbox" style="text-align-last:center;width:5cm;">' + tagsoptions + '</select><br/><br/></td></tr>' + pages + '</table>';
    $( 'body' ).html( body );
    $( '#selTags' ).change( function ()
    {
        $( '#menutable' ).find( 'tr[tags]' ).hide();
        $( '#menutable' ).find( 'tr[tags~=' + $( this ).val() + ']' ).show();
    } ).trigger( 'change' );
    // commong functions to execute if elements exist
    $( 'span[url]' ).click( function ( e ) { window.open( $( this ).attr( 'url' ), "_self" ); } );
    $( 'span[goback]' ).click( function ( e ) { window.history.back(); } );
